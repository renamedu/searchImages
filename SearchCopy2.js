        ...
        return (
          <Group key={index}>
            ...
            {img.similarImages.map((innerImg, innerIndex) => {
              // const isDeleted = !!deletedImages[innerImg.id];
              ...
              return (
                <RichCell
                  ...
                  caption={`similarityPercent: ${innerImg.similarityPercent}%`}
                  actions={
                    <ButtonGroup mode="horizontal" gap="s" stretched>
                      {/* {!isDeleted && <Button mode="secondary" appearance="negative" size="l" onClick={() => handleDeleteClick(innerImg.id)}>
                        <Icon36Delete />
                      </Button>} */}
